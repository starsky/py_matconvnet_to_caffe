...
class TestConv(unittest.TestCase):
    ...
    def before(self, test_dir, rgb2bgr_layer=None):
        work_dir = join(test_dir, 'workspace')
        py_matconv_to_caffe.matconv_to_caffe.convert_model(join(test_dir, 'net.mat'), work_dir, rgb2bgr_layer)
        ...
        if test_img.shape[-1] == 3:
            test_img = test_img[:, :, ::-1] #RGB to BGR
    ...
    def test_verify_vgg16_img(self):
        self.verify('test_data/ucf101-img-vgg16-split1/', rgb2bgr_layer='conv1_1')
    ...
    def test_verify_resnet50_img(self):
        ...
        self.verify('test_data/ucf101-img-resnet-50-split1/', atol=8e-1, rgb2bgr_layer='conv1')
    ...
    def verify(self, test_dir, rgb2bgr_layer=None, atol=1e-2):
        test_net, test_img, reference_values = self.before(test_dir, rgb2bgr_layer=rgb2bgr_layer)
